...
def do_checkpoint(prefix, means, stds):
    def _callback(iter_no, sym, arg, aux):
        ...
        arg['fcis_bbox_weight_test'] = weight * mx.nd.repeat(mx.nd.array(stds), repeats=repeat).reshape((bias.shape[0], 1, 1, 1))
        arg['fcis_bbox_bias_test'] = arg['fcis_bbox_bias'] * mx.nd.repeat(mx.nd.array(stds), repeats=repeat) + mx.nd.repeat(mx.nd.array(means), repeats=repeat)
